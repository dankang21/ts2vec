...
def load_forecast_csv(name, window_in, univar=False):
    data = pd.read_csv(f'datasets/{name}.csv', index_col='datetime', parse_dates=True)
    ...
    if name == 'ETTh1' or name == 'ETTh2':
        ...
        if window_in == 0:
            ...
            train_slice = slice(None, int(0.8 * len(data))-(window_in))
            valid_slice = slice(int(0.8 * len(data))-(window_in), int(1.0 * len(data))-(window_in))
            test_slice = slice(int(1.0 * len(data))-(window_in), None)
            print(test_slice)
